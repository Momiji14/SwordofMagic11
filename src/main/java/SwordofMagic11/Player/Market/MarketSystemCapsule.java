...
public class MarketSystemCapsule extends GUIManager {

    public CustomItemStack icon() {
        CustomItemStack item = new CustomItemStack(Material.CHEST_MINECART);
        item.setDisplay("システムマーケット販売 [カプセル]");
        int index = 0;
        boolean isShort = false;
        ResultSet resultSet = SomSQL.getSqlList(Table.MarketSystemCapsule, "*", "ORDER BY `CapsuleID` ASC");
        for (RowData objects : resultSet) {
            ...
            item.addLore("§7・§f" + capsule.getDisplay() + "§ax" + amount);
            index++;
        ...
        item.setCustomData("Market", "MarketSystemCapsule");
        return item;
    }

    private int scroll = 0;
    private int amount = 0;
    public MarketSystemCapsule(PlayerData playerData) {
        super(playerData, "システムマーケット販売 [カプセル]", 6);
    }
    ...
    public static void set(String id, int amount) {
        if (amount <= 0) {
            ...
            SomSQL.setSql(Table.MarketSystemCapsule, "CapsuleID", id, "Amount", amount);
        }
    }

    public static int get(String id) {
        if (SomSQL.exists(Table.MarketSystemCapsule, "CapsuleID", id)) {
            return SomSQL.getInt(Table.MarketSystemCapsule, "CapsuleID", id, "Amount");
        } else return 0;
    }
    ...
    public static void delete(String id) {
        SomSQL.delete(Table.MarketSystemCapsule, "CapsuleID", id);
    }

    public static int mel(String id) {
        int mel;
        int current = ceil(CapsuleData.Sell * MarketPlayer.MarketSystem);
    ...
    public void updateContainer() {
        ...
        for (RowData objects : resultSet) {
            if (index >= scroll * 8) {
                ...
                item.setCustomData("SystemCapsule", id);
                setItem(slot, item);
                slot++;
    ...
    public void topClick(int slot, ItemStack clickedItem, ClickType clickType) {
        ...
        if (CustomItemStack.hasCustomData(clickedItem, "SystemCapsule")) {
            String id = CustomItemStack.getCustomData(clickedItem, "SystemCapsule");
            CapsuleData capsule = CapsuleDataLoader.getCapsuleData(id);
            ...
            if (amount <= 0) {
                ...
                playerData.addCapsule(capsule, amount);
                playerData.sendMessage(capsule.getDisplay() + "x" + amount + "§aを§b購入§aしました", SomSound.Tick);
                update();
            } else {
